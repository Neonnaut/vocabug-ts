...
class Category_Resolver {
   ...
   public resolve_categories() {
      ...
      for (const [key, value] of this.category_pending) {
         const new_category_field: { graphemes: string[]; weights: number[] } =
            this.resolve_nested_categories(
               value.content,
   ...
   private resolve_nested_categories(
      ...
      function tokenize(
         expr: string,
      ): (string | { group: string; weight: number })[] {
      ...
      function evaluate(expr: string, multiplier = 1): Entry[] {
         ...
         for (let i = 0; i < tokens.length; i++) {
            ...
            if (typeof token === "string") {
               ...
               const has_custom_weight =
                  raw_weight !== undefined && raw_weight !== "";
               const literal_weight = has_custom_weight
                  ? parseFloat(raw_weight)
                  : 1;
               const final_weight = has_custom_weight
